...
if [[ "${JOBS_ID}" == "1" ]]; then
  ...
  for idx in "${!DIRS[@]}"; do
    ...
    $DOCKER exec mydb bash -c "BEARER=$BEARER /home/runner/user_data/ft_client/test_client/maps.sh $ID $JOBS_ID $APP_PATH $DIR_PATH $PARAM_NAME $ARTIFACT $HYPEROPT_PARAM"
