...
if [[ "${JOBS_ID}" == "1" ]]; then
  ...
  if [[ $? -eq 0 ]]; then
    ...
    gh api --method PUT /repos/owner/repo/contents/path/to/file.txt \
      -f message="Update file" \
      -f content="$(base64 -w0 updated_file.txt)" \
      -f sha="$(gh api /repos/owner/repo/contents/path/to/file.txt --jq '.sha')"
